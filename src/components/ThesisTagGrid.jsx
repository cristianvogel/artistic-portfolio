  ...
  return (
    <section className="mt-10 border-t border-neutral-200 pt-6">
      <div className="mb-4 flex items-center gap-4">
        <h3 className="text-[10px] font-mono uppercase tracking-[0.22em] text-neutral-400">Connective Topics</h3>
      ...
      <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
        {tags?.map((tag, index) => (
          <span
            key={tag}
            className={`min-h-16 border border-neutral-200 bg-white/60 px-3 py-3 text-xs leading-tight text-neutral-700 transition duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] hover:border-neutral-900 hover:bg-neutral-950 hover:text-white ${index % 5 === 0 ? 'md:col-span-2' : ''}`}
          >
            {tag}
          </span>
        ))}
        {youtubeInfo && (
          <button
            ...
            className="min-h-16 border border-neutral-950 bg-neutral-950 px-3 py-3 text-left text-xs leading-tight text-white transition duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] hover:bg-white hover:text-neutral-950 md:col-span-2"
          >
            {youtubeInfo.label}
          </button>
        )}
